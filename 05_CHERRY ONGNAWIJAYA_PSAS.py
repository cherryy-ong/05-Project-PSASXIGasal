...
def hal1_login():
    ...
    if password == "12345" and username == "cherryy.ong":
        tampilkan(hal2)
...
def hal2_simpandata():
    email = str(entry3.get())
    film = str(select.get())
    pembayaran = str(status.get())

    if not email or film == "Select a film" or not pembayaran:
...
def hal3_tampilkandata():
    ...
    if data:
        for item in data:
            tabel.insert("", tk.END, values=(item["Email"], item["Film"], item["Jumlah Tiket"], item["Harga"], item["Pembayaran"]))
    else:
...
def hal4_caridata():
    ...
    hasil = [item for item in data if keyword in item["Email"].lower()]

    if hasil:
        for item in hasil:
            tabel_cari.insert("", tk.END, values=(item["Email"], item["Film"], item["Jumlah Tiket"], item["Harga"], item["Pembayaran"]))
    else:
...
label9.pack(pady=5)
label10 = Label(hal4, text="Masukkan Email: ", font=("Times New Roman", 10, "bold"), bg="#BDD5E3")
label10.pack(pady=5)
entry10 = Entry(hal4)
entry10.pack()
...
kolom = ("Email", "Film", "Jumlah Tiket", "Harga", "Pembayaran")
...
judul2.pack(pady=5)
label3 = Label(hal2, text=f"Email: ", font=("Times New Roman", 10, "bold"), bg="#BDD5E3")
label3.pack()
entry3 = Entry(hal2)
entry3.pack()
